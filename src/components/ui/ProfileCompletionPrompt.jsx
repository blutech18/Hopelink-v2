...
const ProfileCompletionPrompt = () => {
  ...
  return (
    <motion.div
      ...
      className="rounded-lg p-4 sm:p-5 mb-6 border border-gray-200 shadow-lg"
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              />
              <span className="text-xs font-medium text-gray-600 bg-gray-50 px-2.5 py-1 rounded-full whitespace-nowrap">
          ...
          {missingFields.length > 0 && (
            <div className="mb-4">
              <p className="text-xs sm:text-sm font-medium text-gray-600 mb-2">Missing Information:</p>
              <div className="flex flex-wrap gap-1.5">
                {missingFields.slice(0, 4).map((field, index) => (
                  <span
                    key={index}
                    className="inline-flex items-center px-2.5 py-1 text-xs bg-gray-50 text-gray-600 rounded-md border border-gray-200"
                  >
                    {field}
                  </span>
                ))}
                {missingFields.length > 4 && (
                  <span className="inline-flex items-center px-2.5 py-1 text-xs bg-gray-50 text-gray-600 rounded-md border border-gray-200 font-medium">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="flex items-center space-x-3 flex-1">
              <div className="flex-1 max-w-[200px] bg-gray-50 rounded-full h-2.5">
                <div
                  className="bg-gradient-to-r from-yellow-400 to-yellow-500 h-2.5 rounded-full transition-all duration-300 shadow-sm"
                  style={{ width: `${completionPercentage}%` }}
                />
              </div>
              <span className="text-xs sm:text-sm font-semibold text-gray-600 whitespace-nowrap">{completionPercentage}%</span>
            </div>
            
            <Link
              to="/profile"
              className="inline-flex items-center justify-center px-4 py-2 text-xs sm:text-sm font-semibold text-gray-900 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-yellow-600 hover:to-yellow-700 rounded-lg transition-all duration-200 shadow-md hover:shadow-lg active:scale-95 whitespace-nowrap"
            >
