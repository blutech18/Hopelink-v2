...
const BrowseRequestsPage = () => {
  ...
  const { requests, isLoading: loading, donorDonations: userDonations, matchingParams, refetchAll } = useBrowseRequests(user?.id)
  ...
  const urgencyLevels = [
    ...
    { value: 'medium', label: 'Medium Priority', color: 'text-blue-500 bg-amber-50' },
  ...
  const requestsFingerprint = useMemo(() => requests.map(r => r.id).join(','), [requests])
  ...
  useEffect(() => {
    ...
    setRequestsWithScores(requests.map(req => ({
      ...req,
      matchingScore: 0,
      bestMatchingDonation: null,
      matchReason: userDonations.length > 0 ? 'Calculating match...' : 'No available donations to match'
    })))
            ...
            try {
              ...
              if (matches && matches.length > 0 && matches[0].score > 0) {
                ...
                if (bestMatch.donation?.donor_id !== user?.id) {
                  return { ...request, matchingScore: 0, bestMatchingDonation: null, matchReason: 'No compatible donations found' }
                }
                if (autoMatchEnabled && bestMatch.score >= autoClaimThreshold) {
                  try {
                    if (bestMatch.donation.status === 'available' && request.status === 'open') {
              ...
              return { ...request, matchingScore: 0, bestMatchingDonation: null, matchReason: 'Unable to calculate match score' }
            }
          })
        )
        if (aborted) return
    ...
    return () => { aborted = true }
  }, [requestsFingerprint, userDonations.length, matchingParams, user?.id, refetchAll])
  ...
  const getScoreColor = (score) => {
    if (score >= 0.9) return 'text-emerald-800 bg-emerald-100 border-emerald-400'
        ...
        >
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-4 gap-3">
            <div className="flex-1">
              <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 mb-2">Browse Requests</h1>
              <div className="flex items-center gap-3 flex-wrap">
                <p className="text-xs sm:text-sm text-gray-600">Find recipients who need your help</p>
                <button
                  type="button"
            ...
            <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
              ...
              <div className="flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-gradient-to-r from-yellow-50 to-yellow-100 border-2 border-yellow-400 rounded-full shadow-sm">
                ...
                <div className="flex items-center gap-1.5">
                  <span className="text-lg sm:text-xl font-bold text-gray-900">{filteredRequests.length}</span>
                  <span className="text-xs sm:text-sm font-medium text-yellow-700">Request{filteredRequests.length !== 1 ? 's' : ''}</span>
                </div>
              </div>
          ...
          {showDonateModal && donateTargetRequest && (
            <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-3 sm:p-4">
              ...
              >
                ...
                <p className="text-gray-600 text-sm mb-4">Request: <span className="text-white font-medium">{donateTargetRequest.title}</span></p>
                ...
                {userDonations && userDonations.length > 0 ? (
                  <div className="space-y-2 max-h-64 overflow-auto custom-scrollbar">
                    {userDonations
                      ...
                      .map(d => (
                      <label key={d.id} className="flex items-start gap-3 p-3 bg-gray-50/50 rounded-lg border border-gray-200 cursor-pointer">
                        <input
                          type="radio"
                        ...
                        <div>
                          ...
                          <div className="text-gray-600 text-xs">Qty: {d.quantity} • {d.category}</div>
                  ...
                  <div className="text-gray-600 text-sm">You have no available donations.
                    ...
                    <div className="mt-2 grid grid-cols-1 gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <label className="text-gray-600 text-xs ml-2">Delivery</label>
            ...
            <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-3 sm:p-4">
              ...
              >
                ...
                <div className="flex justify-between items-center mb-4 sm:mb-6 pb-3 sm:pb-4 border-b-2 border-gray-200">
                  <div className="flex items-center gap-2 sm:gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600 text-xs sm:text-sm">Complete the form to offer your donation</p>
                    </div>
                  </div>
                  <button
                ...
                <div className="space-y-4 sm:space-y-5">
                  ...
                  <div className="bg-gray-50/30 rounded-lg p-4 border border-gray-200">
                    ...
                    <div className="space-y-3">
                      <div>
                        <label className="block text-gray-600 text-xs sm:text-sm font-medium mb-1.5">Donation Title</label>
                        <input 
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          <label className="block text-gray-600 text-xs sm:text-sm font-medium mb-1.5">Quantity</label>
                          <input 
                            type="number" 
                            min="1" 
                    ...
                    <div className="border-2 border-dashed border-gray-200 bg-gray-50/50 rounded-lg p-6 flex flex-col items-center justify-center text-center min-h-[200px]">
                      {!qualityImage ? (
                        <>
                          ...
                          <p className="text-gray-600 text-xs mb-4">Drag & drop or click to upload a clear photo of your donation</p>
                          <input
                            type="file"
                            accept="image/*"
                            className="hidden"
                          ...
                          />
                          <label htmlFor="qualityUpload" className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg cursor-pointer transition-colors flex items-center gap-2">
                        ...
                        <div className="w-full">
                          <div className="relative">
                            <img src={qualityImage} alt="Item preview" className="max-h-80 mx-auto rounded-lg border border-gray-200 shadow-lg" />
                          ...
                          <div className="flex justify-center gap-3 mt-4">
                            <label htmlFor="qualityUpload" className="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-600 text-sm font-medium rounded-lg cursor-pointer transition-colors flex items-center gap-2">
                              <Upload className="h-4 w-4" />
                              Change Photo
                            </label>
                            <input
                ...
                <div className="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3 mt-6 pt-4 border-t-2 border-gray-200">
                  <button 
                    onClick={() => setShowDonationDetailsModal(false)} 
                    className="px-4 sm:px-6 py-2 sm:py-2.5 bg-gray-50 hover:bg-gray-100 text-white text-sm font-medium rounded-lg transition-colors border border-gray-300"
                  >
                    Cancel
                  </button>
                  <button 
                        ...
                        {/* Compact Info Grid */}
                          <div className="grid grid-cols-2 gap-x-4 gap-y-2 mb-3 text-xs">
                            ...
                            <Package className="h-3.5 w-3.5 text-blue-500 flex-shrink-0" />
                              <span className="text-gray-500 font-semibold uppercase tracking-wide text-[10px]">Quantity:</span>
                            ...
                            <User className="h-3.5 w-3.5 text-blue-500 flex-shrink-0" />
                              <span className="text-gray-500 font-semibold uppercase tracking-wide text-[10px]">By:</span>
                            ...
                            <Calendar className="h-3.5 w-3.5 text-purple-500 flex-shrink-0" />
                              <span className="text-gray-500 font-semibold uppercase tracking-wide text-[10px]">Posted:</span>
                          ...
                          {request.needed_by && (
                              ...
                              <Clock className="h-3.5 w-3.5 text-orange-500 flex-shrink-0" />
                                <span className="text-orange-600 font-semibold uppercase tracking-wide text-[10px]">Deadline:</span>
                              <span className="text-orange-700 font-semibold">{formatDate(request.needed_by)}</span>
                            </div>
                          )}
                          ...
                          {(request.location || request.requester) && (
                              ...
                              <MapPin className="h-3.5 w-3.5 text-green-600 flex-shrink-0" />
                                <span className="text-gray-500 font-semibold uppercase tracking-wide text-[10px]">Location:</span>
                      ...
                      {request.tags && request.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mb-3 sm:mb-4">
                          ...
                          {request.tags.length > 4 && (
                            <span className="text-[10px] sm:text-xs text-gray-600 font-medium">+{request.tags.length - 4} more</span>
                          )}
                        </div>
                      )}
                            </div>
                      ...
                      <div className="bg-gray-50/30 rounded-lg p-4 border border-gray-200">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <label className="text-sm font-semibold text-gray-600">Quantity Needed</label>
                          </div>
                        ...
                        <div className="bg-gray-50/30 rounded-lg p-4 border border-gray-200">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              ...
                              <label className="text-sm font-semibold text-gray-600">Needed By</label>
                            </div>
                  ...
                  {loadingProfile ? (
                    ...
                    <>
                        ...
                        <div className="bg-gradient-to-br from-green-500/10 to-emerald-500/10 rounded-lg p-4 border border-green-500/30">
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            ...
                            <div className="bg-gray-50/50 rounded-lg p-2 border border-gray-200">
                              ...
                              <div className="text-lg font-bold text-blue-500">{recipientStats.fulfillmentRate}%</div>
                            </div>
                            <div className="bg-gray-50/50 rounded-lg p-2 border border-gray-200">
                      ...
                      <div className="bg-gray-50/30 rounded-lg p-3 border border-gray-200">
                        ...
                        <p className={`text-xs sm:text-sm leading-relaxed break-words ${selectedRequest.requester?.bio ? 'text-gray-600' : 'text-gray-400 italic'}`}>
