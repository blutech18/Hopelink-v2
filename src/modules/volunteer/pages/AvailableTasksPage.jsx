...
const AvailableTasksPage = () => {
  ...
  const { tasks: rawTasks, isLoading: loading, volunteerProfile, refetchAll } = useAvailableTasks(profile?.id)
  ...
  useEffect(() => {
    ...
    let aborted = false
    const volProfile = volunteerProfile ? { ...profile, ...volunteerProfile } : profile
    ...
    const scoreAndSort = async () => {
        ...
        const batchResults = await Promise.all(
          batch.map(async (task) => {
            try {
              ...
              const matchResult = await intelligentMatcher.calculateTaskScoreForVolunteer(taskWithCategory, volProfile)
              return { ...task, matchingScore: matchResult.score, matchReason: matchResult.matchReason, criteriaScores: matchResult.criteriaScores }
            } catch {
              return { ...task, matchingScore: 0.5, matchReason: 'Unable to calculate match score', criteriaScores: {} }
            }
          })
        )
      ...
      const all = [
        ...scored,
        ...remainingTasks.map(t => ({ ...t, matchingScore: 0.5, matchReason: 'Score pending', criteriaScores: {} }))
      ]
      all.sort((a, b) => {
        if (Math.abs(a.matchingScore - b.matchingScore) > 0.01) return b.matchingScore - a.matchingScore
  ...
  const applyFilters = () => {
    ...
    const tasksToFilter = tasksWithScores.length > 0 ? tasksWithScores : rawTasks
                      ...
                      {task.imageUrl ? (
                        <div className="relative w-full sm:w-56 lg:w-64 h-48 sm:h-56 lg:h-64 rounded-lg overflow-hidden border border-gray-200">
                          ...
                          <div className="w-full sm:w-56 lg:w-64 h-48 sm:h-56 lg:h-64 rounded-lg bg-gradient-to-br from-navy-800 to-navy-900 flex flex-col items-center justify-center border-2 border-gray-300">
                            ...
                            <span className="text-xs text-blue-500 font-semibold mt-1">{task.category}</span>
                          </div>
                        )}
                      </div>
              ...
              <div className="flex items-center justify-between p-6 border-b-2 border-gray-200 flex-shrink-0">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-600">Complete information</p>
                  </div>
                </div>
                <button
                  onClick={() => {
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-gray-50/30 rounded-lg p-4 border border-gray-200">
                      <div className="flex items-center gap-4">
                        <span className="text-gray-600 text-sm font-semibold flex-shrink-0 w-40">Pickup Location</span>
                    ...
                    <div className="bg-gray-50/30 rounded-lg p-4 border border-gray-200">
                      <div className="flex items-center gap-4">
                        <span className="text-gray-600 text-sm font-semibold flex-shrink-0 w-40">Delivery Location</span>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    ...
                    <div className="bg-gray-50/30 rounded-lg p-3 border border-gray-200">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <label className="text-sm font-semibold text-gray-600">Donor Contact</label>
                      </div>
