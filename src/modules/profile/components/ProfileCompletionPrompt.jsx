...
const ProfileCompletionPrompt = () => {
  ...
  const getCompletionStatus = () => {
    ...
    const missingFieldsDisplay = missingRequired.map(
  ...
  const { isComplete, missingFields, completionPercentage } =
    getCompletionStatus();
  ...
  return (
    <motion.div
      ...
      className="rounded-2xl p-4 sm:p-5 mb-6 border border-[#000f3d] dark:border-slate-600 bg-blue-50 dark:bg-slate-800 shadow-sm"
    >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              />
              <span className="text-xs font-medium text-blue-700 dark:text-blue-300 bg-white/80 dark:bg-slate-700/50 border border-blue-100 dark:border-slate-600 px-2.5 py-1 rounded-full whitespace-nowrap">
                {completionPercentage}% Complete
              </span>
            </div>
          </div>

          <p className="text-xs sm:text-sm text-gray-700 dark:text-gray-300 mb-3 leading-relaxed">
            Complete your profile to gain trust from the community and access
            all features. Verified profiles are prioritized for{" "}
            {profile.role === "donor"
              ? "donation requests"
          ...
          {missingFields.length > 0 && (
            <div className="mb-4">
              <p className="text-xs sm:text-sm font-medium text-gray-800 dark:text-gray-200 mb-2">
                Missing information
              </p>
              <div className="flex flex-wrap gap-1.5">
                {missingFields.slice(0, 4).map((field, index) => (
                  <span
                    key={index}
                    className="inline-flex items-center px-2.5 py-1 text-xs bg-white dark:bg-slate-700 text-blue-700 dark:text-blue-300 rounded-md border border-blue-200 dark:border-slate-600"
                  >
                    {field}
                  </span>
                ))}
                {missingFields.length > 4 && (
                  <span className="inline-flex items-center px-2.5 py-1 text-xs bg-white dark:bg-slate-700 text-blue-700 dark:text-blue-300 rounded-md border border-blue-200 dark:border-slate-600 font-medium">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="flex items-center space-x-3 flex-1">
              <div className="flex-1 max-w-[220px] bg-blue-100 dark:bg-slate-700 rounded-full h-2.5">
                <div
                  className="bg-gradient-to-r from-blue-500 to-blue-600 dark:from-blue-600 dark:to-blue-500 h-2.5 rounded-full transition-all duration-300"
                  style={{ width: `${completionPercentage}%` }}
                />
              </div>
              <span className="text-xs sm:text-sm font-semibold text-blue-700 dark:text-blue-300 whitespace-nowrap">
