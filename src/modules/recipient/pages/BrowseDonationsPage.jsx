...
const BrowseDonationsPage = () => {
  ...
  const [showDonorProfileImageModal, setShowDonorProfileImageModal] =
    useState(false);
  ...
  useEffect(() => {
    ...
    if (storedRequests) {
      ...
      const validRequests = new Set(
        [...storedIds].filter((id) => currentDonationIds.has(id)),
      );
  ...
  const donationsFingerprint = useMemo(
      ...
      for (let i = 0; i < donations.length; i += batchSize) {
        ...
        const batchResults = await Promise.all(
          batch.map(async (donation) => {
            try {
              const matches = await intelligentMatcher.matchDonationToRequests(
                donation,
                userRequests,
                recipientProfile,
                1,
                ...
                if (autoMatchEnabled && bestMatch.score >= autoClaimThreshold) {
                  try {
                    if (
                      ...
                      bestMatch.request.status === "open"
                    ) {
                      const hasExistingClaim =
                        donation.claims &&
                        Array.isArray(donation.claims) &&
                        donation.claims.some(
                          (claim) =>
                            claim.recipient_id === user.id &&
                    ...
                    console.warn(
                      `Auto-claim failed for donation ${donation.id}:`,
                      autoClaimErr,
                    );
                  }
                }
  ...
  const handleRequestDonation = async (donation) => {
    ...
    try {
      ...
      success(
        "Request sent successfully! The donor will be notified and can approve your request.",
      );
  ...
  const getScoreColor = (score) => {
    if (score >= 0.8)
      return "text-green-700 bg-green-100 border-green-500 dark:text-green-400 dark:bg-green-900/30 dark:border-green-500/50";
    if (score >= 0.6)
      return "text-blue-700 bg-blue-100 border-blue-500 dark:text-blue-400 dark:bg-blue-900/30 dark:border-blue-500/50";
    if (score >= 0.4)
      return "text-amber-700 bg-amber-100 border-amber-500 dark:text-yellow-400 dark:bg-yellow-900/30 dark:border-yellow-500";
    return "text-orange-700 bg-orange-100 border-orange-500 dark:text-orange-400 dark:bg-orange-900/30 dark:border-orange-500/50";
  };

  const filteredDonations = useMemo(() => {
    const donationsToFilter =
      donationsWithScores.length > 0 ? donationsWithScores : donations;
  ...
  const formatLocation = (donation) => {
    ...
    if (
      isValidLocationValue(donor?.address_landmark) &&
      !isValidLocationValue(donor?.address_street)
    ) {
  ...
  const getDeliveryModeColor = (mode) => {
    switch (mode) {
      ...
      case "volunteer":
        return "text-green-700 bg-green-100 dark:text-green-400 dark:bg-green-900/20";
      case "direct":
        return "text-purple-700 bg-purple-100 dark:text-purple-400 dark:bg-purple-900/20";
      default:
        return "text-gray-700 bg-gray-100 dark:text-gray-400 dark:bg-gray-900/20";
    }
  };
  ...
  const getDeliveryModeLabel = (mode) => {
    switch (mode) {
      ...
      case "organization_pickup":
        return "Organization Pickup";
      default:
        return mode
  ...
  const getDeliveryInstructions = (mode) => {
    switch (mode) {
      case "pickup":
        return "You will need to pick up this donation from the donor.";
      case "volunteer":
        return "A volunteer will coordinate the delivery between you and the donor.";
      case "direct":
        return "The donor will deliver this directly to you.";
      default:
        return "";
    }
  };
              ...
              <div className="flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-gradient-to-r from-yellow-500/20 to-yellow-600/20 border-2 border-yellow-500/40 rounded-full shadow-lg">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs sm:text-sm font-medium text-gray-600">
                    Donation{filteredDonations.length !== 1 ? "s" : ""}
                  </span>
                </div>
              </div>
          ...
          <div className="flex flex-col sm:flex-row flex-wrap gap-3 sm:gap-4">
            ...
            <div className="relative flex-1 min-w-full sm:min-w-[200px]">
              ...
              <input
                ...
                className="w-full pl-9 sm:pl-10 pr-4 py-2 sm:py-3 text-sm sm:text-base bg-gray-50 border-2 border-gray-200 rounded-lg text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all duration-200"
            ...
            <div className="relative w-full sm:w-auto sm:min-w-[180px]">
              <select
                ...
                className="appearance-none w-full px-5 py-3 pr-10 bg-gray-50 border-2 border-gray-200 rounded-lg text-gray-900 dark:text-white font-medium focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all duration-200 cursor-pointer hover:border-yellow-600"
              >
            ...
            <button
              ...
              className={`w-full sm:w-auto px-4 sm:px-5 py-2.5 sm:py-3 rounded-lg text-sm sm:text-base font-semibold transition-all duration-200 flex items-center justify-center gap-2 whitespace-nowrap border-2 active:scale-95 ${
                showUrgentOnly
                  ...
                  : "bg-gray-50 hover:bg-gray-100 text-gray-700 dark:text-gray-200 border-gray-200 hover:border-yellow-600/50"
              }`}
            >
                ...
                >
                  ...
                  {(() => {
                    const showMatchingSection =
                      donation.matchingScore !== undefined &&
                      ...
                      <>
                          ...
                          >
                            <div className="flex items-center justify-between gap-4">
                                ...
                                {donation.bestMatchingRequest && (
                                  <>
                                    ...
                                    <span className="text-xs text-white font-medium truncate">
                                      {donation.bestMatchingRequest.title}
                                    </span>
                                  </>
                                )}
                              ...
                              <div className="flex items-center gap-2 flex-shrink-0">
                                ...
                                <div
                                  className={`px-2.5 py-0.5 rounded text-xs font-bold border ${getScoreColor(donation.matchingScore)}`}
                                >
                        ...
                        >
                              ...
                              <div className="flex items-start justify-between mb-2">
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex flex-wrap gap-1.5 mb-3">
                                    ...
                                    <span
                                      className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold border ${getConditionColor(donation.condition)}`}
                                    >
                                      {donation.condition
                                        ?.replace("_", " ")
                                        .split(" ")
                          ...
                          <div
                            ...
                            style={{
                              backgroundColor: donation.is_urgent
                                ? "#ef4444"
                                : "#fbbf24",
                            }}
                          ></div>
              ...
              >
                ...
                <div className="flex items-center justify-between p-4 sm:p-6 border-b-2 border-gray-200 flex-shrink-0 gap-3">
                  ...
                  {selectedDonation.donor && (
                    <button
                      onClick={() =>
                        handleViewDonorProfile(selectedDonation.donor)
                      }
                  ...
                  <div className="space-y-4 sm:space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="bg-gray-50/30 rounded-lg p-4 border border-gray-200">
                        <div className="flex items-center justify-between">
                          ...
                          <p
                            className={`text-lg font-medium capitalize ${selectedDonation.condition ? "text-gray-900" : "text-gray-600 italic"}`}
                          >
                            {selectedDonation.condition?.replace("_", " ") ||
                              "Not provided"}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="bg-gray-50/30 rounded-lg p-4 border border-gray-200">
                        <div className="flex items-center justify-between">
                          ...
                          <p
                            className={`text-lg font-medium ${selectedDonation.donor?.name ? "text-gray-900" : "text-gray-600 italic"}`}
                          >
                      ...
                      <div className="bg-gray-50/30 rounded-lg p-4 border border-gray-200">
                        <div className="flex items-center justify-between">
                          ...
                          <p
                            className={`text-lg font-medium ${selectedDonation.estimated_value ? "text-gray-900" : "text-gray-600 italic"}`}
                          >
                            {selectedDonation.estimated_value
                              ? `₱${parseInt(selectedDonation.estimated_value).toLocaleString()}`
                              : "Not provided"}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="bg-gray-50/30 rounded-lg p-4 border border-gray-200">
                      <div className="flex items-center justify-between">
                        ...
                        <p
                          className={`text-center max-w-[60%] break-words ${selectedDonation.pickup_location || selectedDonation.donor ? "text-gray-900" : "text-gray-600 italic"}`}
                        >
                          {selectedDonation.pickup_location ||
                          selectedDonation.donor
                            ? formatLocation(selectedDonation)
                            : "Not provided"}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-50/30 rounded-lg p-4 border border-gray-200">
                      <div className="flex items-center justify-between">
                        ...
                        >
                          {selectedDonation.expiry_date
                            ? formatDate(selectedDonation.expiry_date)
                            : "Not provided"}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-50/30 rounded-lg p-4 border border-gray-200">
                      ...
                      selectedDonation.tags.length > 0 ? (
                    ...
                    <div className="bg-amber-50 border border-gray-200 rounded-lg p-4">
                      <p className="text-gray-600 text-sm">
                        <strong>Interested?</strong> Click the "Request
                        Donation" button to send a request to the donor. They
                    ...
                    {loadingDonorProfile ? (
                      ...
                      <>
                        ...
                        <div className="relative flex items-center gap-4">
                          <div className="relative flex-shrink-0">
                            <div
                              ...
                              onClick={() =>
                                setShowDonorProfileImageModal(true)
                              }
                          ...
                          <div className="flex flex-col justify-center min-w-0 flex-1">
                            ...
                            <div className="flex flex-wrap items-center gap-2 text-xs">
                              <span className="text-blue-500 flex items-center gap-1 whitespace-nowrap">
                                ...
                                {(() => {
                                  const memberDate =
                                    ...
                                    selectedDonor?.user_created_at ||
                                    selectedDonor?.joined_at ||
                                    selectedDonor?.signup_date;
                                  if (memberDate) {
                                    try {
                              ...
                              {selectedDonor?.account_type &&
                                selectedDonor.account_type !== "individual" && (
                                  <span className="text-blue-500 flex items-center gap-1 whitespace-nowrap">
                          ...
                          <div className="absolute top-0 right-0 flex-shrink-0">
                            <IDVerificationBadge
                              ...
                              hasIdUploaded={
                                selectedDonor?.primary_id_type &&
                                selectedDonor?.primary_id_number
                              }
                              size="sm"
                              showText={true}
                          ...
                          <div className="bg-gray-50/30 rounded-lg p-3 border border-gray-200">
                            ...
                            <div className="space-y-2 text-xs sm:text-sm">
                              <div className="flex items-start gap-2 min-w-0">
                                ...
                                <span
                                  className={`break-words flex-1 ${selectedDonor?.birthdate ? "text-white" : "text-gray-400 italic"}`}
                                >
                              ...
                              <div className="flex items-start gap-2 min-w-0">
                                ...
                                >
                                  {selectedDonor?.birthdate
                                    ? calculateAge(selectedDonor.birthdate) ||
                                      "Not available"
                                    : "Not provided"}
                                </span>
                              </div>
                              <div className="flex items-start gap-2 min-w-0">
                                ...
                                <span
                                  className={`break-words flex-1 ${selectedDonor?.account_type ? "text-white" : "text-gray-400 italic"}`}
                                >
                                  {selectedDonor?.account_type
                                    ? selectedDonor.account_type === "business"
                              ...
                              {selectedDonor?.account_type === "business" && (
                                <>
                                  <div className="flex items-start gap-2 min-w-0">
                                    ...
                                    <span
                                      className={`break-words flex-1 ${selectedDonor?.organization_name ? "text-white" : "text-gray-400 italic"}`}
                                    >
                                      {selectedDonor?.organization_name ||
                                        "Not provided"}
                                    </span>
                                  </div>
                                  <div className="flex items-start gap-2 min-w-0">
                        ...
                        <div className="bg-gray-50/30 rounded-lg p-3 border border-gray-200">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 text-xs sm:text-sm">
                            <div className="flex items-start gap-2 min-w-0">
                              ...
                              <span
                                className={`break-words flex-1 ${selectedDonor?.address_street ? "text-white" : "text-gray-400 italic"}`}
                              >
                                {selectedDonor?.address_street ||
                                  "Not provided"}
                              </span>
                            </div>
                            <div className="flex items-start gap-2 min-w-0">
                              ...
                              <span
                                className={`break-words flex-1 ${selectedDonor?.address_barangay ? "text-white" : "text-gray-400 italic"}`}
                              >
                            ...
                            <div className="flex items-start gap-2 min-w-0">
                              ...
                              <span
                                className={`break-words flex-1 ${selectedDonor?.address_landmark ? "text-white" : "text-gray-400 italic"}`}
                              >
                                {selectedDonor?.address_landmark ||
                                  "Not provided"}
                              </span>
                            </div>
                            <div className="flex items-start gap-2 min-w-0">
                              ...
                              <span
                                className={`break-words flex-1 ${selectedDonor?.city ? "text-white" : "text-gray-400 italic"}`}
                              >
                            ...
                            <div className="flex items-start gap-2 min-w-0">
                              ...
                              <span
                                className={`break-words flex-1 ${selectedDonor?.province ? "text-white" : "text-gray-400 italic"}`}
                              >
                            ...
                            <div className="flex items-start gap-2 min-w-0">
                              ...
                              <span
                                className={`break-words flex-1 ${selectedDonor?.zip_code ? "text-white" : "text-gray-400 italic"}`}
                              >
                          ...
                          <div className="bg-gray-50/30 rounded-lg p-3 border border-gray-200">
                            ...
                            <div className="space-y-2 text-xs sm:text-sm">
                              <div className="flex items-start gap-2 min-w-0">
                                ...
                                <span
                                  className={`break-words flex-1 ${selectedDonor?.emergency_contact_name ? "text-white" : "text-gray-400 italic"}`}
                                >
                                  {selectedDonor?.emergency_contact_name ||
                                    "Not provided"}
                                </span>
                              </div>
                              <div className="flex items-start gap-2 min-w-0">
                            ...
                            <div className="space-y-2 text-xs sm:text-sm">
                              {selectedDonor?.donation_types?.length > 0 ? (
                                <div className="flex flex-wrap gap-1.5">
                                  {selectedDonor.donation_types.map(
                                    (type, i) => (
                                      <span
                                        key={i}
                        ...
                        <div className="bg-gray-50/30 rounded-lg p-3 border border-gray-200">
                          ...
                          <p
                            className={`text-xs sm:text-sm leading-relaxed break-words ${selectedDonor?.bio ? "text-gray-600" : "text-gray-400 italic"}`}
                          >
